...
public record AbilityVisualDefinition(int schemaVersion, String id, List<HookBinding> bindings) {
    ...
    public AbilityVisualDefinition {
        ...
        List<HookBinding> copy = List.copyOf(Objects.requireNonNull(bindings)); Set<AbilityLifecycleEvent.Hook> hooks=new HashSet<>();
        for (HookBinding entry : copy) {
            ...
            Set<String> ids=new HashSet<>(); for(Emission e:list) if(!ids.add(e.id())) throw new IllegalArgumentException("Duplicate emission id");
        }
        bindings=copy;
    }
